              ...
              <div className="grid md:grid-cols-2 gap-6">
                <div className="space-y-2">
                  ...
                  <input
                    ...
                    className="w-full bg-white/5 border border-white/10 rounded-2xl px-6 py-4 focus:outline-none focus:border-brand-green transition-colors"
                    placeholder={dict.footer.nameLabel}
                  />
                </div>
                <div className="space-y-2">
                  ...
                  <input
                    ...
                    className="w-full bg-white/5 border border-white/10 rounded-2xl px-6 py-4 focus:outline-none focus:border-brand-green transition-colors"
                    placeholder={dict.footer.emailLabel}
                  />
                </div>
              </div>
              <div className="space-y-2">
                ...
                <textarea
                  ...
                  className="w-full bg-white/5 border border-white/10 rounded-2xl px-6 py-4 h-32 focus:outline-none focus:border-brand-green transition-colors"
                  placeholder={dict.footer.messagePlaceholder}
                />
              </div>
              {status === 'success' && (
                <p className="text-brand-green text-sm font-medium">{dict.footer.successMessage || 'Message sent successfully!'}</p>
              )}
              {status === 'error' && (
                <p className="text-red-400 text-sm font-medium">{dict.footer.errorMessage || 'Something went wrong. Please try again.'}</p>
              )}
              <Button
                type="submit"
                disabled={status === 'loading'}
                className="w-full bg-brand-green hover:bg-brand-green/90 text-white rounded-full py-7 text-lg font-semibold shadow-xl shadow-brand-green/40 disabled:opacity-60"
              >
                {status === 'loading' ? '...' : dict.footer.sendButton}
              </Button>
            </form>
          </motion.div>
        </div>
